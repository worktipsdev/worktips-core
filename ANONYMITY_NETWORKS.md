# Anonymity Networks with Worktips
...
`--proxy` which tells `worktipsd` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--proxy tor,127.0.0.1:9050,10`
> `--proxy i2p,127.0.0.1:9000`

which tells `worktipsd` that ".onion" p2p addresses can be forwarded to a socks
...
An ISP can passively monitor `worktipsd` connections from a node and observe when
...
only using I2P for broadcasting Worktips transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of a Worktips
user.
