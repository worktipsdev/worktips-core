...
# Example script using Python to query and decode and decrypt a .worktips address or HF16+ Session address
...
name = "Jason.worktips"
type = 2 # 2 == worktipsnet, 0 == session
...
# Make the RPC request to some worktipsd
...
# For worktipsnet addresses and HF16+ session addresses we'll always have an encrypted value and an
...
# val will currently be the raw worktipsnet ed25519 pubkey (32 bytes).  We can convert it to the more
# common worktipsnet address (which is the same value but encoded in z-base-32) and convert the bytes to
# a string:
...
# Finally slap ".worktips" on the end:
val += ".worktips"
