...
def chuck(net):
    ...
    chuck = Wallet(node=net.nodes[0], name='Chuck', rpc_wallet=net.binpath+'/worktips-wallet-rpc', datadir=net.datadir)
    chuck.ready(wallet="chuck")

    hidden_node = Daemon(worktipsd=net.binpath+'/worktipsd', datadir=net.datadir)
    bridge_node = Daemon(worktipsd=net.binpath+'/worktipsd', datadir=net.datadir)
    for x in (4, 7):
        bridge_node.add_peer(net.all_nodes[x])
    bridge_node.add_peer(hidden_node)
    hidden_node.add_peer(bridge_node)

    vprint("Starting new chuck worktipsd bridge node with RPC on {}:{}".format(bridge_node.listen_ip, bridge_node.rpc_port))
    bridge_node.start()
    bridge_node.wait_for_json_rpc("get_info")
    net.sync(extra_nodes=[bridge_node], extra_wallets=[chuck])

    vprint("Starting new chuck worktipsd hidden node with RPC on {}:{}".format(hidden_node.listen_ip, hidden_node.rpc_port))
    hidden_node.start()
    ...
    chuck.refresh()
    chuck.stop()
    chuck.hidden = Wallet(node=hidden_node, name='Chuck (hidden)', rpc_wallet=net.binpath+'/worktips-wallet-rpc', datadir=net.datadir)
