...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'worktipsd@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [worktipsd] + list(self.__class__.base_args)
        self.args += (
                '--data-dir={}/worktips-{}-{}'.format(datadir or '.', self.listen_ip, self.rpc_port),
                '--log-level={}'.format(log_level),
                '--log-file=worktips.log'.format(self.listen_ip, self.p2p_port),
    ...
    def ping(self, *, storage=True, worktipsnet=True):
        """Sends fake storage server and worktipsnet pings to the running worktipsd"""
        if storage:
            self.json_rpc("storage_server_ping", { "version_major": 9, "version_minor": 9, "version_patch": 9 })
        if worktipsnet:
            self.json_rpc("worktipsnet_ping", { "version": [9,9,9] })
...
class Wallet(RPCDaemon):
    ...
    def __init__(
            ...
            node,
            *,
            rpc_wallet='worktips-wallet-rpc',
            name=None,
            datadir=None,
            listen_ip=None,
            rpc_port=None,
