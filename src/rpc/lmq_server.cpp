  ...
  : core_{core}, rpc_{rpc}
{
...
#ifndef _WIN32
    locals.push_back("ipc://" + core.get_config_directory() + "/worktipsd.sock");
#endif
  ...
  //
  lmq.add_request_command("sub", "mempool", [this](worktipsmq::Message& m) {
  ...
  // encoded block hash).
  lmq.add_request_command("sub", "block", [this](worktipsmq::Message& m) {
      std::unique_lock lock{subs_mutex_};
    auto expiry = std::chrono::steady_clock::now() + 30min;
