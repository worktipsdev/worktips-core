...
// Copyright (c) 2019, The uPlexa Developers
...
void cn_upx_hash(const void *data, size_t length, char *hash, int light, int variant, int prehashed, uint32_t scratchpad, uint32_t iterations)
{
  ...
  RDATA_ALIGN16 uint8_t hp_state[CN_UPX_PAGE_SIZE];
#else
#warning "ACTIVATING FORCE_USE_HEAP IN aarch64 + crypto in slow-hash.c"
  uint8_t *hp_state = (uint8_t *)aligned_malloc(CN_UPX_PAGE_SIZE,16);
#endif
...
#else
#warning "ACTIVATING FORCE_USE_HEAP IN aarch64 && !crypto in slow-hash.c"
  uint8_t *long_state = (uint8_t *)malloc(CN_UPX_PAGE_SIZE);
#endif
