...
bool bind(sql_compiled_statement& s, int index, worktipsmq::string_view text)
{
...
// from a string_view
bool bind_blob(sql_compiled_statement& s, int index, worktipsmq::string_view blob)
{
...
template <typename T, std::enable_if_t<std::is_same<T, worktipsmq::string_view>::value, int> = 0>
worktipsmq::string_view get(sql_compiled_statement& s, int index)
{
...
worktipsmq::string_view get_blob(sql_compiled_statement& s, int index)
{
...
worktipsmq::string_view get_blob(sql_compiled_statement& s, I index)
{
...
mapping_record sql_get_mapping_from_statement(sql_compiled_statement& statement)
{
  ...
  {
    auto value = get<worktipsmq::string_view>(statement, mapping_record_column::encrypted_value);
  ...
  // Copy name hash
  {
    auto value = get<worktipsmq::string_view>(statement, mapping_record_column::name_hash);
...
bool sql_compiled_statement::compile(worktipsmq::string_view query, bool optimise_for_multiple_usage)
{
  sqlite3_stmt* st;
...
uint64_t expiry_blocks(cryptonote::network_type nettype, mapping_type type, uint64_t *renew_window)
{
  ...
  if (is_worktipsnet_type(type))
  {
    ...
    if (type == mapping_type::worktipsnet_1year)        result = BLOCKS_EXPECTED_IN_YEARS(1);
    else if (type == mapping_type::worktipsnet_2years)  result = BLOCKS_EXPECTED_IN_YEARS(2);
    else if (type == mapping_type::worktipsnet_5years)  result = BLOCKS_EXPECTED_IN_YEARS(5);
    else if (type == mapping_type::worktipsnet_10years) result = BLOCKS_EXPECTED_IN_YEARS(10);
...
bool validate_lns_name(mapping_type type, std::string name, std::string *reason)
{
  ...
  if (is_worktipsnet)                         max_name_len = lns::WORKTIPSNET_DOMAIN_NAME_MAX;
  ...
  if (is_worktipsnet)
  {
    ...
    if (check_condition(name == "localhost.worktips", reason, "LNS type=", type, ", specifies mapping from name->value using protocol reserved name=", name))
      return false;
    ...
    char const SHORTEST_DOMAIN[] = "a.worktips";
    if (check_condition((name.size() < static_cast<int>(worktips::char_count(SHORTEST_DOMAIN))), reason, "LNS type=", type, ", specifies mapping from name->value where the name is shorter than the shortest possible name=", SHORTEST_DOMAIN, ", given name=", name))
      return false;

    // Must end with .worktips
    char const SUFFIX[]     = ".worktips";
    char const *name_suffix = name.data() + (name.size() - worktips::char_count(SUFFIX));
    if (check_condition((memcmp(name_suffix, SUFFIX, worktips::char_count(SUFFIX)) != 0), reason, "LNS type=", type, ", specifies mapping from name->value where the name does not end with the domain .worktips, name=", name))
      return false;

    // Characted preceeding suffix must be alphanumeric
    char const *char_preceeding_suffix = name_suffix - 1;
    if (check_condition(!char_is_alphanum(char_preceeding_suffix[0]), reason, "LNS type=", type ,", specifies mapping from name->value where the character preceeding the <char>.worktips is not alphanumeric, char=", char_preceeding_suffix[0], ", name=", name))
      return false;
...
bool validate_mapping_value(cryptonote::network_type nettype, mapping_type type, std::string const &value, mapping_value *blob, std::string *reason)
{
  ...
  {
    int max_value_len = 0;
    if (is_worktipsnet_type(type))              max_value_len = (WORKTIPSNET_ADDRESS_BINARY_LENGTH * 2);
  ...
  {
    if (check_condition(value.size() != 52, reason, "The worktipsnet value=", value, ", should be a 52 char base32z string, length=", value.size()))
      return false;
...
bool validate_encrypted_mapping_value(mapping_type type, std::string const &value, std::string *reason)
{
  ...
  if (is_worktipsnet_type(type)) max_value_len              += WORKTIPSNET_ADDRESS_BINARY_LENGTH;
...
static bool validate_against_previous_mapping(lns::name_system_db &lns_db, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_worktips_name_system const &lns_extra, std::string *reason = nullptr)
{
  std::stringstream err_stream;
  ...
  if (mapping)
  {
    ...
    {
      if (check_condition(is_worktipsnet_type(lns_extra.type) && !mapping.active(lns_db.network_type(), blockchain_height), reason, tx, ", ", lns_extra_string(lns_db.network_type(), lns_extra), " TX requested to update mapping that has already expired"))
        return false;
    ...
    {
      if (!is_worktipsnet_type(lns_extra.type))
      {
        if (check_condition(true, reason, tx, ", ", lns_extra_string(lns_db.network_type(), lns_extra), " non-worktipsnet entries can NOT be renewed, mapping already exists with name_hash=", mapping.name_hash, ", owner=", mapping.owner.to_string(lns_db.network_type()), ", type=", mapping.type))
          return false;
      }
      ...
      {
        // Worktipsnet entry expired i.e. it's no longer active. A purchase for this name is valid
...
bool name_system_db::validate_lns_tx(uint8_t hf_version, uint64_t blockchain_height, cryptonote::transaction const &tx, cryptonote::tx_extra_worktips_name_system *lns_extra, std::string *reason)
{
  ...
  // -----------------------------------------------------------------------------------------------
  cryptonote::tx_extra_worktips_name_system lns_extra_;
  {
    ...
    if (check_condition(!cryptonote::get_worktips_name_system_from_tx_extra(tx.extra, *lns_extra), reason, tx, ", didn't have worktips name service in the tx_extra"))
      return false;
  }
...
bool encrypt_mapping_value(std::string const &name, mapping_value const &value, mapping_value &encrypted_value)
{
  ...
  static_assert(mapping_value::BUFFER_SIZE >= WORKTIPSNET_ADDRESS_BINARY_LENGTH    + crypto_secretbox_MACBYTES, "Value blob assumes the largest size required, all other values should be able to fit into this buffer");
...
static int64_t add_or_get_owner_id(lns::name_system_db &lns_db, crypto::hash const &tx_hash, cryptonote::tx_extra_worktips_name_system const &entry, lns::generic_owner const &key)
{
  int64_t result = 0;
...
static bool add_lns_entry(lns::name_system_db &lns_db, uint64_t height, cryptonote::tx_extra_worktips_name_system const &entry, crypto::hash const &tx_hash)
{
  // -----------------------------------------------------------------------------------------------
    ...
    sql_compiled_statement statement{lns_db};
    {
      ...
      for (size_t i = 0; i < column_count; i++)
      {
        ...
        switch (column_type)
        {
          ...
          case mapping_record_column::name_hash:       bind(statement, i+1, worktipsmq::string_view{name_hash}); break;
...
static bool get_txid_lns_entry(cryptonote::Blockchain const &blockchain, crypto::hash txid, cryptonote::tx_extra_worktips_name_system &extra)
{
  ...
  return cryptonote::get_worktips_name_system_from_tx_extra(txs[0].extra, extra);
}
...
static std::vector<replay_lns_tx> find_lns_txs_to_replay(cryptonote::Blockchain const &blockchain, lns::mapping_record const &mapping, uint64_t blockchain_height)
{
  ...
  */
  ...
  {
    cryptonote::tx_extra_worktips_name_system curr_lns_extra = {};
...
bool name_system_db::save_mapping(crypto::hash const &tx_hash, cryptonote::tx_extra_worktips_name_system const &src, uint64_t height, int64_t owner_id, int64_t backup_owner_id)
{
  if (!src.is_buying())
    return false;
